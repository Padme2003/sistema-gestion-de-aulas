...
'Mobiliario (mesas, sillas, escritorio del docente)',
'Equipos tecnológicos (computadora, proyector)',
'Periféricos (teclado, mouse, parlantes)',
'Red e Internet (router, punto de red, cableado)',
'Iluminación (luminarias, interruptores)',
  ];
      ...
      this.apiService.getUsuarios().subscribe({
        next: (usuarios) => {
          ...
          if (usuarioEncontrado && usuarioEncontrado.idUsuario) {
            ...
            this.currentUser = usuarioEncontrado.nombre;
          ...
          // Cargar asignaciones y solicitudes solo después de actualizar el ID
          this.loadAsignaciones();
          this.loadMisSolicitudes();
        },
        error: (err) => {
          console.error('Error obteniendo usuarios:', err);
          // Al menos cargamos asignaciones aunque falle
          this.loadAsignaciones();
        }
      });
    } else {
      // Si el ID ya es correcto
      this.loadAsignaciones();
      this.loadMisSolicitudes();
    }
  } else {
